    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if csv_file.name.endswith('.csv'):
                # Parse CSV file and add data to database
                decoded_file = csv_file.read().decode('utf-8').splitlines()
                csv_reader = csv.DictReader(decoded_file)
                
                for row in csv_reader:
                    # Assuming your CSV has columns like 'employee_id', 'date', 'time_in', 'time_out'
                    # Adjust the field names according to your CSV structure
                    Employee_ID = row['ID'] #di ko alam pano ito XD 
                    Name = row['name']
                    
                    # Create AttendanceEntry objects and save to database
